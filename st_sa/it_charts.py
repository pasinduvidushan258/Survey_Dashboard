...
def draw_overall_satisfaction_donut(df, col_name):
    ...
    with ui.card().classes('w-full p-4 shadow-sm border rounded-xl bg-white h-full flex flex-col'):
        ui.label('Overall satisfaction of ICT facilities').classes('font-bold text-[#0b1132] mb-2')
        ui.echart({
...
def draw_dynamic_faculty_bar(df, questions_dict, fac_col):
    with ui.card().classes('w-full p-4 shadow-sm border rounded-xl bg-white h-full flex flex-col'):
        ui.label('Satisfaction of ICT Facilities by faculty wise').classes('font-bold text-xl text-[#0b1132] mb-4')
...
def draw_quick_stats_boxes(df, questions_dict):
    # 🔴 මෙතන flex-wrap සහ min-w දාලා තියෙන්නේ ප්‍රශ්න 16ම ලස්සනට පිළිවෙලට වැටෙන්න

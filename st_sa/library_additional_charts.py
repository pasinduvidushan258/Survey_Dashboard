...
from st_sa.library_charts import _get_multi_pct_with_count, COLOR_MAP, _get_faculty_short
...
def draw_year_comparison_bar(df, all_cols, year_col):
    ...
    y_vals = []
    for y in years:
        match = data.get(y, {'pct': 0, 'count': 0})
    ...
    with ui.card().classes('w-full p-4 shadow-sm border rounded-xl bg-white h-full flex flex-col'):
        ...
        ui.echart({
            ...
            'xAxis': {'type': 'category', 'data': years}, 'yAxis': {'max': 100},
...
def draw_clustered_faculty_year(df, all_cols, fac_col, year_col):
    ...
    for f in active_facs:
        ...
        series.append({'name': f, 'type': 'bar', 'data': f_data, 'itemStyle': {'color': COLOR_MAP.get(f, '#9CA3AF')}})
    with ui.card().classes('w-full p-4 shadow-sm border rounded-xl bg-white h-full flex flex-col'):

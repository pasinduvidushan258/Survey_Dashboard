...
from st_sa.medical_charts import _get_multi_pct_with_count, COLOR_MAP, _get_faculty_short
...
def draw_year_comparison_bar(df, all_cols, year_col):
    ...
    with ui.card().classes('w-full p-4 shadow-sm border rounded-xl bg-white h-full flex flex-col'):
        ...
        ui.label('💡 This chart shows the trend of overall medical center satisfaction across different years.').classes('text-xs text-gray-500 mt-auto text-center pt-2')
...
def draw_clustered_faculty_year(df, all_cols, fac_col, year_col):
    ...
    for f in facs:
        f_data = []
        for y in years:
            val = next((v['pct'] for k, v in data.items() if str(k[0])==y and _get_faculty_short(k[1])==f), 0)
            f_data.append(val)
        series.append({'name': f, 'type': 'bar', 'data': f_data, 'itemStyle': {'color': COLOR_MAP.get(f, '#000000'), 'borderRadius': [3,3,0,0]}, 'label': {'show': False}})
    ...
    with ui.card().classes('w-full p-4 shadow-sm border rounded-xl bg-white h-full flex flex-col'):
        ...
        ui.label('💡 This chart breaks down the medical center satisfaction percentage by each faculty, grouped by year.').classes('text-xs text-gray-500 mt-auto text-center pt-2')